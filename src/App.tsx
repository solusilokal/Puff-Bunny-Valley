    ...
    <>
      <main className="w-full max-w-[480px] mx-auto relative shadow-2xl bg-[#E8F4F8] min-h-screen overflow-hidden pb-32">
        ...
        <section className="relative w-full min-h-[90dvh] flex flex-col justify-end pb-12 px-6">
          ...
          <button
            onClick={handleShare}
            className="absolute top-6 right-6 z-20 p-3 bg-white/30 backdrop-blur-md rounded-full border border-white/40 text-white hover:bg-white/40 transition-all shadow-sm cursor-pointer"
            aria-label="Bagikan"
          >
          ...
          <div className="relative z-10 flex flex-col items-center text-center mt-32 bg-white/90 backdrop-blur-md p-6 rounded-3xl shadow-xl border border-[#BDE0B8] mx-2">
            ...
            <button 
              ...
              className="cursor-pointer group flex items-center justify-center gap-2 w-full py-4 bg-[#F29F58] text-white rounded-full font-bold text-[15px] shadow-[0_8px_20px_rgba(242,159,88,0.4)] hover:bg-[#E08A46] hover:-translate-y-1 transition-all border-2 border-[#E08A46]/20"
            >
              <Ticket size={20} />
        ...
        <section className="py-10 px-6 bg-white border-y border-[#D5E8C9]">
          ...
          <div className="mt-8 flex overflow-x-auto snap-x snap-mandatory gap-3 pb-4 no-scrollbar">
            {pageData.galleryPhotos.map((img, idx) => (
              <div key={idx} onClick={() => openLightbox(pageData.galleryPhotos, idx)} className="snap-center shrink-0 w-[160px] aspect-[3/4] rounded-2xl overflow-hidden cursor-pointer shadow-sm border-2 border-white">
                <img src={img} alt={`Keseruan ${idx + 1}`} className="w-full h-full object-cover hover:scale-110 transition-transform duration-500" />
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="py-10 px-6">
          <div className="bg-[#4A5D4E] rounded-3xl p-6 text-white relative overflow-hidden shadow-lg">
            ...
            <div className="relative z-10 flex flex-col gap-4 text-sm text-slate-200">
              ...
              <a href={pageData.links.maps} target="_blank" rel="noreferrer" className="text-center py-3 w-full bg-[#78A962] text-white font-bold rounded-xl mt-2 hover:bg-[#689454] transition-colors border-2 border-[#8AC071]/50 block">
                Buka di Google Maps
              </a>
            </div>
          </div>
            ...
            {pageData.faq.map((item, idx) => (
              <div key={idx} className="border border-[#D5E8C9] rounded-2xl overflow-hidden bg-[#F4F9EB]/50">
                <button 
                  ...
                  className="w-full flex justify-between items-center p-4 text-left hover:bg-[#F4F9EB] transition-colors cursor-pointer"
                >
        ...
        <section id="booking-form" className="py-12 px-6">
          <div className="bg-[#78A962] rounded-[2rem] p-6 shadow-xl relative overflow-hidden border-2 border-[#8AC071]">
            ...
            <form onSubmit={handleFormSubmit} className="flex flex-col gap-4 relative z-10 bg-[#F4F9EB] p-5 rounded-2xl shadow-sm border border-[#D5E8C9]">
              ...
              <button 
                type="submit"
                className="w-full mt-2 bg-[#F29F58] text-white font-bold text-[15px] py-3.5 rounded-xl flex items-center justify-center gap-2 hover:bg-[#E08A46] transition-colors shadow-md border-2 border-[#E08A46]/20 cursor-pointer"
              >
        ...
        >
          <button 
            onClick={scrollToForm}
            className="w-full flex items-center justify-between px-6 py-4 bg-[#F29F58] backdrop-blur-xl rounded-full text-white shadow-[0_10px_30px_rgba(242,159,88,0.4)] hover:scale-[1.02] active:scale-[0.98] transition-all border border-white/20 cursor-pointer"
          >
      ...
      {lightbox.isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-sm" onClick={closeLightbox}>
          <button className="absolute top-6 right-6 p-2 bg-white/10 rounded-full text-white hover:bg-white/20 z-50 cursor-pointer" onClick={closeLightbox}>
            <X size={24} />
          </button>
          {lightbox.images.length > 1 && (
            <button className="absolute left-4 p-3 bg-white/10 rounded-full text-white hover:bg-white/20 z-50 cursor-pointer" onClick={prevImage}>
              <ChevronLeft size={28} />
            </button>
          )}
          ...
          {lightbox.images.length > 1 && (
            <button className="absolute right-4 p-3 bg-white/10 rounded-full text-white hover:bg-white/20 z-50 cursor-pointer" onClick={nextImage}>
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 backdrop-blur-sm transition-opacity" onClick={() => setShowShareModal(false)}>
          <div className="w-full max-w-[480px] bg-white rounded-t-[2rem] p-6 relative" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={() => setShowShareModal(false)} className="p-2 bg-slate-100 rounded-full text-slate-500 cursor-pointer">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4 mb-6">
              <div className="flex flex-col items-center gap-2">
                <button onClick={copyToClipboard} className="w-14 h-14 rounded-full bg-slate-100 flex items-center justify-center text-slate-600 hover:bg-slate-200 transition-colors cursor-pointer">
                  {copied ? <Check size={24} className="text-green-500" /> : <Copy size={24} />}
                </button>
                <span className="text-[11px] font-bold text-slate-500">{copied ? 'Tersalin!' : 'Salin'}</span>
              </div>
              <div className="flex flex-col items-center gap-2">
                <button onClick={() => window.open(`https://wa.me/?text=${encodeURIComponent(window.location.href)}`)} className="w-14 h-14 rounded-full bg-[#25D366] flex items-center justify-center text-white hover:opacity-90 transition-opacity cursor-pointer">
                  <MessageCircle size={24} className="fill-current" />
                </button>
                <span className="text-[11px] font-bold text-slate-500">WhatsApp</span>
              </div>
              <div className="flex flex-col items-center gap-2">
                <button onClick={() => window.open(`https://twitter.com/intent/tweet?url=${encodeURIComponent(window.location.href)}`)} className="w-14 h-14 rounded-full bg-black flex items-center justify-center text-white hover:opacity-90 transition-opacity cursor-pointer">
                  <Twitter size={24} />
                </button>
...
function ChevronDown({ className, size = 20 }: { className?: string; size?: number }) {
  return (
